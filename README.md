...
3. Greiti pakeitimai atliekami `config.json` faile: atnaujinkite CSV nuorodas, rezervinius CSV ir skaičiavimo parametrus pagal poreikį.
...
- Duomenų šaltinis, demonstraciniai įrašai, papildomas istorinis CSV ir stulpelių atitikmenys – `config.json` `dataSource` ir `csv` blokuose. Istoriniam rinkiniui pakanka stulpelių **„Numeris“**, **„Atvykimo data“**, **„Išrašymo data“**, **„Siuntimas“**, **„GMP“**, **„Nukreiptas į padalinį“** – „Diena/naktis“ gali nebūti, nes paros metas apskaičiuojamas iš atvykimo laiko.
...
- Rezervinį duomenų rinkinį galite išjungti `config.json` faile (nustatykite `useFallback: false`), jei norite matyti tik realią klaidos būseną.
...
6. (Pasirinktinai) Laikinai atjunkite internetą – statusas turi pereiti į oranžinę žinutę apie demonstracinius duomenis, konsolėje matysite klaidos detalizaciją.
7. (Pasirinktinai) `config.json` nustatykite `useFallback: false` ir pakartokite 6 žingsnį – statusas turi tapti raudonas su konkrečiu klaidos aprašu.
