...
import { parseColorToRgb, relativeLuminance, rgbToRgba } from './runtime/utils/color.js';
...
export function startFullPageApp(options = {}) {
  const forcePageId =
    typeof options?.forcePageId === 'string' ? options.forcePageId.trim().toLowerCase() : '';
  ...
  const renderCharts = (dailyStats, funnelTotals, heatmapData) =>
    chartRenderers.renderCharts(dailyStats, funnelTotals, heatmapData);
  const edRenderBridge = createEdRenderBridgeFeature({
    getEdRenderer: () => edRenderer,
    getChartRenderers: () => chartRenderers,
  });
  const { renderEdDashboard, renderEdDispositionsChart } = edRenderBridge;
  ...
  const { handleChartCopyClick, handleChartDownloadClick, handleTableDownloadClick } = copyExportFeature;
  ...
  function populateHeatmapMetricOptions() {
    ...
    const current =
      typeof dashboardState?.heatmapMetric === 'string'
  ...
  function populateHeatmapYearOptions(dailyStats) {
    ...
    const current = Number.isFinite(dashboardState.heatmapYear) ? String(dashboardState.heatmapYear) : 'all';
  ...
  function renderArrivalHeatmap(container, heatmapData, accentColor, metricKey = DEFAULT_HEATMAP_METRIC) {
    ...
    matrix.forEach((rowValues, dayIndex) => {
      ...
      rowValues.forEach((value, hourIndex) => {
        ...
        const durationSamples = Array.isArray(countsMatrix?.[dayIndex])
          ? countsMatrix[dayIndex][hourIndex]
          : 0;
        const hasCellData =
  ...
  function _computeShiftDateKey(referenceDate, shiftStartHour) {
  ...
  function aggregatePeriodSummary(entries) {
    ...
    return entries.reduce(
      (acc, entry) => {
        ...
        const hospitalizedDurations = Number.isFinite(entry?.hospitalizedDurations)
  ...
  function filterRecordsByChartFilters(records, filters) {
    ...
    return (Array.isArray(records) ? records : []).filter((record) =>
      recordMatchesChartFilters(record, normalized)
    );
  }
  ...
  function _renderKpiPeriodSummary(lastShiftSummary, periodMetrics) {
  ...
  function createEdSkeletonCard(cardConfig) {
    ...
    const type =
      cardConfig?.type === 'donut' ? 'donut' : cardConfig?.type === 'comments' ? 'comments' : 'default';
  ...
  function showEdSkeleton() {
    ...
    sections.forEach((section) => {
      fragment.appendChild(createEdSkeletonSection(section));
    });
  ...
  function _renderDailyChart(dailyStats, period, ChartLib, palette) {
    return chartRenderers.renderDailyChart(dailyStats, period, ChartLib, palette);
  }

  function _renderHourlyChart(records, ChartLib, palette) {
  ...
  function setActiveTab(tabId, { focusPanel = false, restoreFocus = false } = {}) {
    ...
    if (selectors.tabButtons?.length) {
    ...
    if (selectors.edNavButton) {
      ...
      const panelLabel =
        getDatasetValue(selectors.edNavButton, 'panelLabel') || settings?.output?.tabEdLabel || TEXT.tabs.ed;
      const openLabel =
        getDatasetValue(selectors.edNavButton, 'openLabel') ||
        (typeof TEXT.edToggle?.open === 'function'
          ? TEXT.edToggle.open(panelLabel)
          : `Atidaryti ${panelLabel}`);
      const closeLabel =
        getDatasetValue(selectors.edNavButton, 'closeLabel') ||
    ...
    if (focusPanel) {
      ...
      if (targetPanel && typeof targetPanel.focus === 'function') {
        ...
        normalized === 'ed' &&
        selectors.edHeading &&
        typeof selectors.edHeading.scrollIntoView === 'function'
      ) {
  ...
  function computeFunnelStats(dailyStats, targetYear, fallbackDailyStats) {
    ...
    const aggregated = scoped.reduce(
      (acc, item) => ({
        ...
        hospitalized:
          acc.hospitalized + (Number.isFinite(item.entry?.hospitalized) ? item.entry.hospitalized : 0),
  ...
  function computeArrivalHeatmap(records) {
    ...
    aggregates.forEach((row, dayIndex) => {
      ...
      const _durationDiv = weekdayDays.avgDuration[dayIndex].size || 1;
  ...
  function enrichSummaryWithOverviewFallback(summary, overviewRecords, overviewDailyStats, options = {}) {
    ...
    records.forEach((record) => {
      ...
      if (arrival && discharge) {
        ...
        if (Number.isFinite(diffMinutes) && diffMinutes >= 0) {
          ...
          if (dateKey) {
            const bucket = losDailyBuckets.get(dateKey) || {
              dateKey,
              fastCount: 0,
              slowCount: 0,
              losCount: 0,
    ...
    if (!hasPeakWindow) {
      ...
      if (topArrivalHours.length || topDepartureHours.length) {
        ...
        const hasRiskNote =
          typeof summary.peakWindowRiskNote === 'string' && summary.peakWindowRiskNote.trim().length;
        if (topArrivalHours.length && topDepartureHours.length) {
    ...
    if (losValues.length) {
      ...
      if (
        (!summary.losPercentilesText || !summary.losPercentilesText.trim()) &&
      ...
      if (!Number.isFinite(avgDaily) && dailySource.length) {
        ...
        const totals = effectiveDaily.reduce(
          (acc, entry) => {
      ...
      if (
        !Number.isFinite(summary.littlesLawEstimate) &&
    ...
    if (needsFastSlow && (losValidCount > 0 || losDailyBuckets.size > 0)) {
      ...
      if (
        (!summary.fastSlowSplitValue || !summary.fastSlowSplitValue.trim()) &&
        Number.isFinite(summary.fastLaneShare) &&
        Number.isFinite(summary.slowLaneShare)
      ) {
      ...
      if (previousAgg.totalLos > 0 && recentAgg.totalLos > 0) {
        ...
        fastDelta =
          Number.isFinite(previousFastShare) && Number.isFinite(recentFastShare)
            ? recentFastShare - previousFastShare
            : null;
        slowDelta =
          Number.isFinite(previousSlowShare) && Number.isFinite(recentSlowShare)
      ...
      if ((!summary.fastSlowTrendText || !summary.fastSlowTrendText.trim()) && losValidCount > 0) {
        summary.fastSlowTrendText =
          'Langas: visi turimi duomenys • Pokyčiams apskaičiuoti reikia bent 2 langų.';
      }
    }
  ...
  function filterDailyStatsByYear(dailyStats, year) {
    ...
    return (Array.isArray(dailyStats) ? dailyStats : []).filter((entry) => {
      ...
      const date = dateKeyToDate(entry.date);
      return date instanceof Date && !Number.isNaN(date.getTime()) && date.getUTCFullYear() === targetYear;
    });
  }
  ...
  function showChartError(message) {
    ...
    const fallbackMessage =
      TEXT?.charts?.errorLoading || TEXT?.status?.error || 'Nepavyko atvaizduoti grafikų.';
    const resolvedMessage = message && String(message).trim().length ? String(message) : fallbackMessage;
    hideChartSkeletons();
    selectors.chartCards.forEach((card) => {
      if (!card) {
        return;
  ...
  function _areStylesheetsLoaded() {
  ...
  function _computeVisibleRatio(rect) {
    if (!rect) {
      return 0;
    }
  ...
  function normalizeSettings(rawSettings) {
    ...
    merged.dataSource.historical.label =
      merged.dataSource.historical.label != null
    ...
    [
      ...
      merged.csv[key] =
        merged.csv[key] != null ? String(merged.csv[key]) : String(DEFAULT_SETTINGS.csv[key] ?? '');
    });
    ...
    merged.output.pageTitle =
      merged.output.pageTitle != null ? String(merged.output.pageTitle) : DEFAULT_SETTINGS.output.pageTitle;
    merged.output.title =
      merged.output.title != null ? String(merged.output.title) : DEFAULT_SETTINGS.output.title;
    merged.output.subtitle =
      merged.output.subtitle != null ? String(merged.output.subtitle) : DEFAULT_SETTINGS.output.subtitle;
    merged.output.kpiTitle =
      merged.output.kpiTitle != null ? String(merged.output.kpiTitle) : DEFAULT_SETTINGS.output.kpiTitle;
    merged.output.kpiSubtitle =
      merged.output.kpiSubtitle != null
        ? String(merged.output.kpiSubtitle)
        : DEFAULT_SETTINGS.output.kpiSubtitle;
    merged.output.chartsTitle =
      merged.output.chartsTitle != null
        ? String(merged.output.chartsTitle)
        : DEFAULT_SETTINGS.output.chartsTitle;
    merged.output.chartsSubtitle =
      merged.output.chartsSubtitle != null
        ? String(merged.output.chartsSubtitle)
        : DEFAULT_SETTINGS.output.chartsSubtitle;
    merged.output.recentTitle =
      merged.output.recentTitle != null
        ? String(merged.output.recentTitle)
        : DEFAULT_SETTINGS.output.recentTitle;
    merged.output.recentSubtitle =
      merged.output.recentSubtitle != null
        ? String(merged.output.recentSubtitle)
        : DEFAULT_SETTINGS.output.recentSubtitle;
    ...
    merged.output.monthlyTitle =
      merged.output.monthlyTitle != null
        ? String(merged.output.monthlyTitle)
        : DEFAULT_SETTINGS.output.monthlyTitle;
    merged.output.monthlySubtitle =
      merged.output.monthlySubtitle != null
        ? String(merged.output.monthlySubtitle)
        : DEFAULT_SETTINGS.output.monthlySubtitle;
    merged.output.yearlyTitle =
      merged.output.yearlyTitle != null
        ? String(merged.output.yearlyTitle)
        : DEFAULT_SETTINGS.output.yearlyTitle;
    merged.output.yearlySubtitle =
      merged.output.yearlySubtitle != null
        ? String(merged.output.yearlySubtitle)
        : DEFAULT_SETTINGS.output.yearlySubtitle;
    merged.output.feedbackTitle =
      merged.output.feedbackTitle != null
        ? String(merged.output.feedbackTitle)
        : DEFAULT_SETTINGS.output.feedbackTitle;
    merged.output.feedbackSubtitle =
      merged.output.feedbackSubtitle != null
        ? String(merged.output.feedbackSubtitle)
        : DEFAULT_SETTINGS.output.feedbackSubtitle;
    ...
    merged.output.footerSource =
      merged.output.footerSource != null
        ? String(merged.output.footerSource)
        : DEFAULT_SETTINGS.output.footerSource;
    merged.output.scrollTopLabel =
      merged.output.scrollTopLabel != null
        ? String(merged.output.scrollTopLabel)
        : DEFAULT_SETTINGS.output.scrollTopLabel;
    merged.output.tabOverviewLabel =
      merged.output.tabOverviewLabel != null
        ? String(merged.output.tabOverviewLabel)
        : DEFAULT_SETTINGS.output.tabOverviewLabel;
    merged.output.tabEdLabel =
      merged.output.tabEdLabel != null
        ? String(merged.output.tabEdLabel)
        : DEFAULT_SETTINGS.output.tabEdLabel;
    merged.output.edTitle =
      merged.output.edTitle != null ? String(merged.output.edTitle) : DEFAULT_SETTINGS.output.edTitle;
  ...
  function applySettingsToText() {
    ...
    TEXT.feedback.trend.title =
      settings.output.feedbackTrendTitle || DEFAULT_SETTINGS.output.feedbackTrendTitle;
  ...
  function _buildCsvRuntime(csvSettings) {
  ...
  function _detectHospitalized(value, csvRuntime) {
  ...
  const { resetEdCommentRotation, renderEdCommentsCard } = edCommentsFeature;
  ...
  const { formatEdCardValue, buildFeedbackTrendInfo, buildEdCardVisuals } = edCardsFeature;
  ...
  const { buildEdStatus, createEdSectionIcon, normalizeEdSearchQuery, matchesEdSearch, applyEdSearchFilter } =
    edPanelCoreFeature;
  ...
  const { applyTextContent } = textContentFeature;
  ...
  function createChunkReporter(_label) {
    return (payload = {}) => {
      if (statusDisplay.loading) {
        ...
        const current = Number.isFinite(payload.current)
          ? payload.current
          : Number.isFinite(payload.receivedBytes)
  ...
  function setStatus(type, details = '') {
    ...
    if (dashboardState.usingFallback) {
      ...
      const warningsList = Array.isArray(dashboardState.dataMeta?.warnings)
        ? dashboardState.dataMeta.warnings.filter(
  ...
  const kpiFlow = createKpiFlow({
    ...
    renderLastShiftHourlyChartWithTheme: (seriesInfo) =>
      chartRenderers.renderLastShiftHourlyChartWithTheme(seriesInfo),
    setChartCardMessage,
    getSettings: () => settings,
  ...
  function _toDateKeyFromDate(date) {
    if (!(date instanceof Date) || Number.isNaN(date.getTime())) {
      return '';
    }
  ...
  function _toMonthKeyFromDate(date) {
    if (!(date instanceof Date) || Number.isNaN(date.getTime())) {
      return '';
    }
    const year = date.getFullYear();
  ...
  function _resolveNightBoundsMinutes(calculationSettings = {}) {
  ...
  function _isNightTimestamp(date, nightStartMinutes, nightEndMinutes) {
    if (!(date instanceof Date) || Number.isNaN(date.getTime())) {
      return null;
    }
  ...
  function rerenderChartsForTheme() {
    ...
    renderFeedbackTrendChart(feedbackMonthly).catch((error) => {
      const errorInfo = describeError(error, {
        code: 'FEEDBACK_TREND_THEME',
        message: 'Nepavyko perpiešti atsiliepimų trendo grafiko pakeitus temą',
      });
    ...
    const edHasSnapshot =
      Number.isFinite(edSummary?.currentPatients) ||
    ...
    const edVariant = edMode === 'snapshot' || (edMode === 'hybrid' && edHasSnapshot) ? 'snapshot' : 'legacy';
    ...
    renderEdDispositionsChart(
      ...
      const errorInfo = describeError(error, {
        code: 'ED_DISPOSITIONS_THEME',
        message: 'Nepavyko perpiešti pacientų kategorijų grafiko pakeitus temą',
      });
      console.error(errorInfo.log, error);
    });
    if (dashboardState.kpi?.lastShiftHourly) {
      chartRenderers
        .renderLastShiftHourlyChartWithTheme(dashboardState.kpi.lastShiftHourly)
        .catch((error) => {
          const errorInfo = describeError(error, {
            code: 'LAST_SHIFT_THEME',
            message: 'Nepavyko perpiešti paskutinės pamainos grafiko pakeitus temą',
          });
          console.error(errorInfo.log, error);
        });
    }
    const hasAnyData =
      dashboardState.chartData.dailyWindow?.length ||
    ...
    renderCharts(
      dashboardState.chartData.dailyWindow,
      dashboardState.chartData.funnel,
      dashboardState.chartData.heatmap
    ).catch((error) => {
      const errorInfo = describeError(error, {
        code: 'CHARTS_THEME',
        message: 'Nepavyko perpiešti grafikų pakeitus temą',
      });
      console.error(errorInfo.log, error);
  ...
  function createTrendChangeCell(diff, percentChange, maxAbsDiff, canCompare = true, variant = 'yearly') {
    ...
    if (!canCompare || !Number.isFinite(diff)) {
      const unavailableText =
        (variant === 'monthly' ? TEXT.monthly?.comparisonUnavailable : TEXT.yearly?.comparisonUnavailable) ||
    ...
    const width = direction === 'neutral' ? 0 : Math.min(100, Math.max(8, Math.round(normalized)));
  ...
  function buildMonthlySparkline(series, highlights = []) {
    ...
    const highlightKeys = Array.isArray(highlights)
      ? highlights.map((key) => (typeof key === 'string' ? key.replace(/^monthly-/, '') : '')).filter(Boolean)
      : [];
    ...
    const baseColor =
      computedStyles.getPropertyValue('--color-accent-soft').trim() || 'rgba(37, 99, 235, 0.2)';
  ...
  function renderMonthlyComparison(newer, older) {
    ...
    if (
      Number.isFinite(newerMeta.year) &&
      Number.isFinite(olderMeta.year) &&
      newerMeta.year !== olderMeta.year
    ) {
  ...
  function renderRecentTable(recentDailyStats) {
    ...
    const _hospShareList = sorted.map((entry) => {
      const total = Number.isFinite(entry?.count) ? entry.count : 0;
  ...
  function buildHospitalStayCell({ count, pct, unclassified = false }) {
    ...
    wrapper.className = unclassified
      ? 'charts-hospital-cell charts-hospital-cell--unclassified'
  ...
  function renderChartsHospitalTable(records = dashboardState.rawRecords) {
    ...
    rows.forEach((entry) => {
      ...
      if (
        normalizeChartsHospitalTableDepartment(dashboardState.chartsHospitalTableDepartment) ===
      ...
      unclassifiedCell.appendChild(
        buildHospitalStayCell({
      ...
      row.append(departmentCell, lt4Cell, from4To8Cell, from8To16Cell, gt16Cell, unclassifiedCell, totalCell);
      selectors.chartsHospitalTableBody.appendChild(row);
    });
  ...
  function renderMonthlyTable(monthlyStats) {
    ...
    const maxAbsDiff = diffValues.reduce(
    ...
    scopedMonthly.forEach((entry, index) => {
      ...
      const previousYearEntry = previousYearKey
        ...
        : null;
      const previousYearTotal = Number.isFinite(previousYearEntry?.count)
        ? previousYearEntry.count
        : Number.NaN;
      ...
      const yoyComparison = formatMonthlyYoYComparison(
        total,
        previousYearTotal,
        isComplete && previousYearComplete
      );
      row.innerHTML = `
  ...
  function renderYearlyTable(yearlyStats) {
    ...
    const entriesToRender =
      Number.isFinite(displayLimit) && displayLimit > 0 ? yearlyStats.slice(-displayLimit) : yearlyStats;
    ...
    const latestYear = entriesToRender.length ? entriesToRender[entriesToRender.length - 1].year : null;
    ...
    entriesToRender.forEach((entry, index) => {
      ...
      const monthMaxAbsDiff = monthDiffs.reduce(
        (acc, value) => (Number.isFinite(value) ? Math.max(acc, Math.abs(value)) : acc),
        0
      );
  ...
  const { scheduleInitialLoad } = createDataFlow({
    pageConfig,
    selectors,
    dashboardState,
    TEXT,
